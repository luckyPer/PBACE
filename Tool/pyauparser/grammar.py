...
def get_enum_name(cls, value):
    return [k for k, v in list(cls.__dict__.items()) if v == value][0]
...
class Grammar(object):
    ...
    def _load(f):
        ...
        def read_short():
            #c = f.read(2)
            c1 = f.read(1)
            c1 = struct.unpack("c", c1)[0]
            c2 = f.read(1)
            c2 = struct.unpack("c", c2)[0]
            return ord(c1) + ord(c2) * 256 if len(c1) + len(c2) == 2 else None
    ...
    def _link_reference(self):
        ...
        for g in list(self.symbolgroups.values()):
        ...
        for s in list(self.lalrstates.values()):
        ...
        self.symbol_EOF = [s for s in list(self.symbols.values()) if s.type == SymbolType.END_OF_FILE][0]
        self.symbol_Error = [s for s in list(self.symbols.values()) if s.type == SymbolType.ERROR][0]
